...
- 开发阶段采用轻量流程：新实施任务各用独立分支／worktree，完成聚焦验证后默认提交本任务、自动合入本地 `dev`，并从干净主 checkout 构建安装 OpenCode Lab（debug），无需用户再说“发 debug”；明确的“先别提交／合并”“先别发包”等限制优先。现有在途工作按原授权收尾，独立答疑／诊断仍只读。
