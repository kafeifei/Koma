...
- Lab 的桌面、Web 和 CLI 都连接同一 profile 的本地后端。执行、权限、归档恢复、删除和目录占用判定由后端完成。关闭窗口或断开 Web／CLI 不停止后端；完整退出或重启桌面 App 时，核对已连接后端身份后停止该后端及其任务（包括 CLI 任务），等待释放 profile 后再完成退出。下次需要启动后端时使用当前安装包的可执行文件。Lab 复用现有 HTTP 服务和 Session 引擎，不修改上游执行循环。终端自己的显示配置与 UI 插件仍属于终端。
