...
- **项目与目录**：运行时项目 ID 在仓库首次打开时生成并持久保存于 Git 公共目录的 `opencode` 文件，已有 ID 原样沿用；远端地址、提交和目录改名不重算身份。Linked Worktree 共用项目 ID，新克隆独立分配；非 Git 目录沿用 `global` 的目录作用域。Session 的 `project_id` 表示运行时项目，`directory` 表示执行目录。工作台的项目是可移除的目录入口，显示归属单独按 server scope 与 Session ID 保存；无项目会话归入不可移除的“最近”分组，重新添加目录不自动改变旧会话归属。不同 server 的项目与任务保持隔离，显示名不是标识。分支选择与是否使用隔离 Worktree 独立；切换视图、移除项目都不取消执行或删除目录、任务。
