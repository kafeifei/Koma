...
## Model and permission preference follow-up (2026-09-07, source verified)

New tasks remember the last explicitly selected model, reasoning level, and permission mode for the current server and project. OpenCode and Codex retain their own settings. The remembered engine follows the user's model selection. Existing drafts keep their explicit choices; existing sessions remain owned by their backend state. Preference hydration must not rewrite an in-flight native request or turn a temporary catalog failure into a new saved default. Codex model display, submission, and exact retries keep their existing behavior; this follow-up does not add model fallback semantics.

This adds presentation preferences for new-task creation, not permission policy or automatic approval. Validation covers refresh, close/reopen, a subsequent new task after sending, project separation, and preservation of existing-session settings.

Production sidebar-switch comparison used the existing performance fixture with three cold and three hot trials per run. Baseline median first-correct/stable was 50.3/66.8 ms cold and 16.1/24.5 ms hot; the candidate measured 58.7/76.4 ms cold and 19.1/22.7 ms hot. Both runs passed with zero wrong, blank, unknown, or source samples. This is a limited local comparison, not a statistical performance claim; it does not cover new-input hydration.

Final checks with Bun 1.3.14 passed: App and E2E typechecks, three preference unit tests, all 18 input-retention browser regressions, and all 11 Codex-composer regressions. The shared input fixture now gives the second project its own backend identity; the earlier fixture mapped every directory to the first project. The existing Codex draft regression now explicitly selects OpenCode for the second input after verifying same-project engine inheritance.

An isolated browser and backend run verified OpenCode model/variant/permission after refresh, close/reopen, first submission and another new task, including the actual local model-provider request and persisted Session settings. Separate Codex catalog and create-response fixtures verified model/effort/permission recovery and the submitted native API payload, with both rejection retention and successful-first-send/new-task coverage. A second project kept its defaults, and an existing OpenCode Session retained its backend settings after the new input changed to Codex. No real Codex account/provider execution or installed-app runtime validation is claimed by these source checks.
