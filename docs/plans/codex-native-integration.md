...
不导入用户已有 Codex/Sandy 全量历史，不暴露跨引擎 fork/revert。对未支持的操作显式关闭能力，不能调用原 OpenCode 实现完成一个语义不同的动作。

永久删除沿用 Session 删除入口，由宿主按 engine 分派到 Codex Host。界面须同时取得 Session 删除能力和外部 descriptor 的删除能力；旧后端未报告能力时不开放。任务及其已知子任务须全部通过预检，可信绑定须重新确认原生空闲；活动 turn、工具、待处理交互或结果不明的投递均阻止开始删除。只有确定从未开始创建原生 thread 的记录可以直接清理；不以目录或标题推断未知绑定。

删除意图持久保存到现有外部绑定，原子阻止新输入投递及新增子任务；受管 worktree 仍沿用既有预检、快照和占用保护。按子任务到父任务的顺序确认原生 `thread/delete` 结果，再删除本地 Session 并发布现有删除事件，最后收尾专属 worktree。原生删除非幂等，回执丢失时保留任务及删除状态，重试先核实原生是否仍存在；不能仅隐藏 Lab 记录并宣称原生历史已删除。按 runtime scope 与原生 thread ID 保存独立于 Session 行的确认删除回执，父任务历史重放只跳过有精确回执的子任务引用，其余读取失败仍须报告。未投递队列随明确的永久删除一并清理。
