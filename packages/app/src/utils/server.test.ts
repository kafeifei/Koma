...
describe("extension management transport", () => {
  test("uses the selected server and credentials and preserves project and package identities", async () => {
    ...
    await api.extensions.catalog()
    await api.extensions.catalog(true)
    await api.extensions.installCatalog("plugin:https://github.com/kdcokenny/opencode-background-agents")
    ...
    expect(new URL(requests[1]!.url).pathname).toBe("/global/extensions/plugins/remove")
    expect(await requests[1]!.json()).toEqual({ id: "@scope/plugin" })
    expect(new URL(requests[2]!.url).searchParams.get("directory")).toBe("/projects/A & B")
    expect(new URL(requests[3]!.url).pathname).toBe("/global/extensions/catalog")
    expect(requests[3]!.method).toBe("GET")
    expect(new URL(requests[4]!.url).pathname).toBe("/global/extensions/catalog/refresh")
    expect(requests[4]!.method).toBe("POST")
    expect(new URL(requests[5]!.url).pathname).toBe("/global/extensions/catalog/install")
    expect(await requests[5]!.json()).toEqual({ id: "plugin:https://github.com/kdcokenny/opencode-background-agents" })
