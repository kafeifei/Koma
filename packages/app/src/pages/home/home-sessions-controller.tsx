...
export function createHomeSessionsController(home: HomeController) {
  ...
  const allRecords = createMemo(() =>
    buildHomeSessionRecords({
      ...
      projectByID,
      assignments: () => home.server.focusedContext()?.projects.assignments() ?? {},
      knownProjects: () => home.server.focusedSync().data.project,
    }),
  )
      ...
      open: (session: Session, options?: OpenSessionOptions) => {
        ...
        const project =
          home.project
            ...
            ) ?? projectForSession(session, home.project.list(), projectByID(), home.server.focusedSync().data.project)
        const conn = home.server.focused()
        if (!conn) return
