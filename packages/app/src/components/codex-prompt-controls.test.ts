...
describe("Codex prompt settings", () => {
  ...
  test("requires an available Provider model, including for old native-only tasks", () => {
    expect(canSubmitWithProviderModel({ provider: { id: "xd", name: "XD" } })).toBeTrue()
    expect(canSubmitWithProviderModel({})).toBeFalse()
    expect(canSubmitWithProviderModel(undefined)).toBeFalse()
  })
})
