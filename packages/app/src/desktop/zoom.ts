...
type ZoomAdapter = {
  ...
  onZoomFactorChanged(callback: (factor: number) => void): unknown
  getPinchZoomEnabled(): Promise<boolean>
  setPinchZoomEnabled(enabled: boolean): Promise<void>
  onPinchZoomEnabledChanged(callback: (enabled: boolean) => void): unknown
}

export function createDesktopZoom(adapter: ZoomAdapter) {
  const OS_NAME = (() => {
  ...
  adapter.onZoomFactorChanged((factor) => {
    requestedZoom = clamp(factor)
    setWebviewZoom(requestedZoom)
  })

  void adapter.getPinchZoomEnabled().then((enabled) => {
    pinchZoomEnabled = enabled
  })

  adapter.onPinchZoomEnabledChanged((enabled) => {
    pinchZoomEnabled = enabled
    resetWheelPinch()
  })
  ...
  const setPinchZoomEnabled = (enabled: boolean) => {
    pinchZoomEnabled = enabled
    resetWheelPinch()
    return adapter.setPinchZoomEnabled(enabled)
  }
  ...
  return { webviewZoom, resetZoom, setPinchZoomEnabled, zoomIn, zoomOut }
}
