...
export type DesktopShutdownDialog = {
  type: "warning"
  title: string
  message: string
  detail: string
...
export type DesktopBackendConnection = { url: string; username?: string; password?: string }

export async function confirmDesktopShutdown(input: {
  translate(key: DesktopNativeKey): string
  backend(): Promise<DesktopBackendConnection | undefined>
  showDialog(options: DesktopShutdownDialog): Promise<{ response: number }>
    ...
    const response = await (input.fetch ?? fetch)(new URL("/lab/shutdown-state", backend.url), {
      headers:
        backend.username && backend.password
          ? { Authorization: `Basic ${btoa(`${backend.username}:${backend.password}`)}` }
          : {},
      signal: abort.signal,
    })
  ...
  const result = await input.showDialog({
    type: "warning",
    title: input.translate("desktop.shutdown.title"),
    message: input.translate(unknown ? "desktop.shutdown.unknown.message" : "desktop.shutdown.active.message"),
    detail: input.translate(unknown ? "desktop.shutdown.unknown.detail" : "desktop.shutdown.active.detail"),
    buttons: [
      input.translate("desktop.shutdown.cancel"),
      input.translate(unknown ? "desktop.shutdown.quitAnyway" : "desktop.shutdown.stopAndQuit"),
    ],
    defaultId: 0,
    cancelId: 0,
    noLink: true,
