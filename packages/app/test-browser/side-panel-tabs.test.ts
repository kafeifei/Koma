...
import { BACKGROUND_TASKS_TAB, childSessionTab, terminalTab } from "../src/pages/session/side-panel-tabs"

test("side content reacts to selection and layout changes without becoming a file tab", () => {
  createRoot((dispose) => {
    const [state, setState] = createStore({
      enabled: true,
      active: BACKGROUND_TASKS_TAB,
      all: ["file://src/a.ts", BACKGROUND_TASKS_TAB, childSessionTab("ses_child"), terminalTab("pty_1")],
    })
    ...
    expect(tabs.activeTab()).toBe(BACKGROUND_TASKS_TAB)
    expect(tabs.activeFileTab()).toBeUndefined()
    expect(tabs.closableTab()).toBe(BACKGROUND_TASKS_TAB)
