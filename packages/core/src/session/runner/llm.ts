...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadSystemContext = (agent: AgentV2.Selection, model: string) =>
      Effect.all(
        [labInstructions.context(model), systemContext.load(), skillGuidance.load(agent), referenceGuidance.load()],
    ...
    const runTurnAttempt = Effect.fn("SessionRunner.runTurn")(function* (
      ...
      const toolMaterialization = isLastStep
        ? undefined
        : yield* tools.materialize(session.permissionMode === "full" ? [] : agent.info?.permissions)
    ...
    const run = Effect.fn("SessionRunner.run")(function* (input: {
      ...
      yield* SessionEngineGuard.requireOpenCode(db, input.sessionID, "runner").pipe(Effect.orDie)
