...
// Recorded worktrees and relocation aliases, including archived owners, keep their old logical directory.
export function resolve(directory: string, root = process.env.OPENCODE_HOME?.trim()) {
  ...
  const worktrees = [...(metadata?.worktrees ?? []), ...(metadata?.directoryAliases ?? [])]
