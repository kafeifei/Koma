...
按用户确认，同一渠道的 Electron 与 Tauri 使用同一份 Koma CLI、共享 Koma 数据目录，但各自启动独立后端与端口。Tauri 固定 `KOMA_BACKEND_INSTANCE=tauri`；数据路径由 Koma CLI 解析：显式 `KOMA_HOME`／`OPENCODE_HOME` 优先，macOS 发行默认 `~/Library/Application Support/Koma/profile`，仅 Debug 保留 `~/.koma` 兼容规则。发行版不自动导入 Lab／Debug 的数据与凭据，不绑定测试 worktree。
