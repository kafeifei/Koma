...
按用户确认，Electron 与 Tauri 使用同一份 Koma CLI、共享 Koma 数据目录，但各自启动独立后端与端口。Tauri 固定 `KOMA_BACKEND_INSTANCE=tauri`；数据路径由 Koma CLI 解析：显式 `KOMA_HOME`／`OPENCODE_HOME` 优先，发行版与 Debug 默认均为 `~/.koma`，保留已有 Lab profile 的物理目录、锁与兼容链接。数据共用不改变各宿主的凭据服务身份，不绑定测试 worktree。
