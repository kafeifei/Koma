...
test("self and sibling host connections are rejected while sharing is disabled", async () => {
  ...
  try {
    ...
    const cleanup = await input.controller.remove(["use1/hello-world", "use1/other-device"])
    expect(cleanup.results.map((item) => item.status)).toEqual(["protected", "protected"])
    expect(cleanup.state.clientID).toBe("test-device")
    expect(cleanup.state.devices.every((device) => device.current)).toBe(true)
  } finally {
    await input.controller.stop()
  }
})
...
test("a late disconnect notification from a deleted client cannot evict its replacement", async () => {
  ...
  try {
    ...
    const connected = async () =>
      (await input.controller.getState()).devices.find((device) => device.id === "use1/other-device")?.connected
