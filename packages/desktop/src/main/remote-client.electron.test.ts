...
for (const clientOrigin of ["oc://renderer", "tauri://localhost"])
  test.skipIf(process.env.OPENCODE_ELECTRON_TEST !== "1")(
    `proxies ${clientOrigin} through both gateways in the packaged Node runtime`,
    async () => {
      ...
      import { createWebEntry } from ${JSON.stringify(resolve(import.meta.dir, "web-entry.ts"))}
      ...
      const backend = createServer((request, response) => {
        assert.equal(request.headers.authorization, "Basic " + Buffer.from("opencode:test").toString("base64"))
      ...
      const host = createWebEntry({
        backend: async () => ({ url: "http://127.0.0.1:" + backendPort, username: "opencode", password: "test" }),
        root,
        remoteOrigin: () => device.url,
      })
      const hostEntry = await host.start()
      const hostPort = Number(new URL(hostEntry.url).port)
      const relay = (connect) => ({
      ...
      const connection = await connector(async () => {
        ...
        return createConnection(hostPort, "127.0.0.1")
      })({ management: {}, id: device.id, root, clientOrigin: ${JSON.stringify(clientOrigin)} })
      const headers = { origin: ${JSON.stringify(clientOrigin)}, "sec-fetch-site": "cross-site", "sec-fetch-mode": "cors", "sec-fetch-dest": "empty" }
      for (const url of [connection.url, hostEntry.url]) {
        assert.equal(await fetch(url + "/api", { headers: { origin: "https://attacker.example", "sec-fetch-site": "cross-site" } }).then(r => r.status), 403)
      }
      const response = await fetch(connection.url + "/api", { headers })
      assert.equal(await response.text(), "private")
      assert.equal(response.headers.get("access-control-allow-origin"), ${JSON.stringify(clientOrigin)})
      ...
      function upgrade(url) {
        ...
        return new Promise((resolve, reject) => {
          ...
          socket.on("connect", () => socket.write(
            "GET /ws HTTP/1.1\\r\\nHost: " + target.host +
            "\\r\\nOrigin: " + ${JSON.stringify(clientOrigin)} + "\\r\\nSec-Fetch-Site: cross-site" +
            "\\r\\nConnection: Upgrade\\r\\nUpgrade: websocket\\r\\n\\r\\n",
          ))
